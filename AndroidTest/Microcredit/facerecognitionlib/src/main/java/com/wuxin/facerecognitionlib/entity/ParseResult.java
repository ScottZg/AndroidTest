		...
		try {
			...
			for(int i=0; i<items.length(); i++) {
				...
				try {
					...
					JSONObject mouthTop = landmark.getJSONObject("mouth_upper_lip_top");
					JSONObject mouthBottom = landmark.getJSONObject("mouth_lower_lip_bottom");
					
					rect[i].mouth_upper_lip_top = new Point(mouthTop.getInt("x"), mouthTop.getInt("y"));
					rect[i].mouth_lower_lip_bottom = new Point(mouthBottom.getInt("x"), mouthBottom.getInt("y"));
